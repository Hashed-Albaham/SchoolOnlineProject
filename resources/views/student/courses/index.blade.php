    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-luxury-400 text-sm mt-1">{{ $courses->total() ?? 0 }}
                    {{ __('site.available_courses_count') }}
                </p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-luxury-800/50 backdrop-blur-xl border border-white/5 rounded-2xl p-6 mb-8">
                <form action="{{ route('student.courses.index') }}" method="GET"
                    ...
                    <div class="flex-1 relative">
                        <svg class="absolute right-4 rtl:right-4 ltr:left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-luxury-400"
                    ...
                    <select name="sort"
                        ...
                        <option value="price_low" {{ request('sort') === 'price_low' ? 'selected' : '' }}>
                            {{ __('site.sort_price_low') }}
                        </option>
                        <option value="price_high" {{ request('sort') === 'price_high' ? 'selected' : '' }}>
                            {{ __('site.sort_price_high') }}
                        </option>
                    </select>
                ...
                <div class="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                    @foreach($courses as $course)
                        <a href="{{ route('student.courses.show', $course) }}" class="group">
                            <div
                                class="bg-luxury-800/50 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden card-luxury">
                                <!-- Thumbnail -->
                                <div
                                    ...
                                    @endif
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-luxury-900/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                    </div>
                                ...
                                <div class="p-5">
                                    <h3
                                        class="font-semibold text-white line-clamp-2 mb-2 group-hover:text-gold-400 transition min-h-[3rem]">
                                        {{ $course->title }}
                                    </h3>
                                    ...
                                    <div class="flex items-center justify-between text-sm">
                                        <div class="flex items-center gap-1 text-luxury-400">
                                            ...
                                            <span>{{ $course->contents_count ?? 0 }} {{ __('site.lessons_count') }}</span>
                                        </div>
                                        <div class="flex items-center gap-1 text-luxury-400">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span>{{ $course->enrollments_count ?? 0 }}
                                                {{ __('site.students_count') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
